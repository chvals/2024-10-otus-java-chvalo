...
public class MessageController {
    ...
    private static final String TOPIC_TEMPLATE_ROOM_ID = "/topic/response.";
    private static final String TOPIC_TEMPLATE_ROOM_ALL = "/topic/responseRoomAll";
    ...
    public void getMessageByRoomId(@DestinationVariable("roomId") String roomId, Message message) {
        logger.info("get message:{}, roomId:{}", message, roomId);
        saveMessage(roomId, message).subscribe(msgId -> logger.info("message send id:{}", msgId));

        template.convertAndSend(
                String.format("%s%s", TOPIC_TEMPLATE_ROOM_ID, roomId),
                new Message(HtmlUtils.htmlEscape(message.messageStr())));
        template.convertAndSend(
                String.format("%s", TOPIC_TEMPLATE_ROOM_ALL), new Message(HtmlUtils.htmlEscape(message.messageStr())));
    }
    ...
    public void handleSessionSubscribeEvent(SessionSubscribeEvent event) {
        ...
        if (simpDestination.startsWith(TOPIC_TEMPLATE_ROOM_ID)) {
        ...
        if (simpDestination.startsWith(TOPIC_TEMPLATE_ROOM_ALL)) {
            logger.info("subscription for:{}, roomId:{}, user:{}", simpDestination, "all", principal.getName());
            // /user/f6532733-51db-4d0e-bd00-1267dddc7b21/topic/responseRoomAll
            getMessages()
                    .doOnError(ex -> logger.error("getting messages for roomId:{} failed", "all", ex))
                    .subscribe(message -> template.convertAndSendToUser(principal.getName(), simpDestination, message));
        }
    }
    ...
    private long parseRoomId(String simpDestination) {
        try {
            var idxRoom = simpDestination.lastIndexOf(TOPIC_TEMPLATE_ROOM_ID);
            return Long.parseLong(simpDestination.substring(idxRoom).replace(TOPIC_TEMPLATE_ROOM_ID, ""));
        } catch (Exception ex) {
